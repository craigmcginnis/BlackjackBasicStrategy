...
export class StrategyChartComponent {
    ...
    effect(()=>{
      ...
      if(rs){
        ...
        this.ruleNotes = [
          ...
          rs.doubleAfterSplit ? 'DAS: Doubling allowed after a split' : 'No DAS: Doubling after split not allowed',
          rs.lateSurrender ? 'LS: Late surrender available (after dealer checks for blackjack)' : 'No LS: Surrender not available'
        ];
  ...
  dLabel(v:number){ return v===11?'A':v; }
  cls(d: string){ return d === '?' ? '' : d.toLowerCase(); }
  ...
  private rebuildHardGroups(){
    ...
    const startTotal = 5; const endTotal = 21;
    ...
    this.hardDisplay = groups.map(g=> ({ label: g.totals.length>1 ? `${g.totals[0]}-${g.totals[g.totals.length-1]}` : String(g.totals[0]), totals: g.totals }));
  }
}
