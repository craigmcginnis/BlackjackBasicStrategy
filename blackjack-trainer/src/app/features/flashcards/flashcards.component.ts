...
interface FlashItem { label: string; expected: string; type:'hard'|'soft'|'pair'; playerCards:{rank:string;value:number;suit:string}[]; dealerUp:{rank:string;value:number;suit:string}; key:string; }
...
export class FlashcardsComponent {
  ...
  get totalTracked(){ return Object.keys(this.srs).length; }
  get currentEf(){ return this.current && this.srs[this.current.key] ? this.srs[this.current.key].ef.toFixed(2) : '—'; }
  get nextDue(){ return this.current && this.srs[this.current.key] ? this.relativeTime(this.srs[this.current.key].nextDue) : '—'; }
  ...
  rebuild(){
    this.items = []; this.index = 0; this.build();
    // apply SRS due filtering & sorting
    const now = Date.now();
    const srsMap = this.srs || {} as Record<string,{consecutive:number;intervalIndex:number;nextDue:number}>;
    ...
    due.sort((a,b)=>{
      const ea = srsMap[a.key]; const eb = srsMap[b.key];
      const nda = ea?ea.nextDue:0; const ndb = eb?eb.nextDue:0;
      if(nda !== ndb) return nda - ndb;
      const ca = ea?ea.consecutive:0; const cb = eb?eb.consecutive:0;
      return ca - cb;
    });
    this.items = [...due, ...future];
    this.shuffle();
    this.current = this.items[0]; this.startTs = performance.now();
  }
  ...
  build(){
    ...
    Object.keys(this.tables.hard).forEach(totalStr => {
      ...
      Object.keys(row).forEach(dStr => {
        ...
        this.items.push({ key, label: `Hard ${total} vs ${d===11?'A':d}`, expected: row[d], type:'hard', playerCards: combo, dealerUp });
      });
    });
    // Soft totals
    Object.keys(this.tables.soft).forEach(k => {
      ...
      Object.keys(row).forEach(dStr => {
        ...
        this.items.push({ key, label: `Soft ${softVal} (${k}) vs ${d===11?'A':d}`, expected: row[d], type:'soft', playerCards: combo, dealerUp });
      });
    });
    // Pairs
    Object.keys(this.tables.pairs).forEach(p => {
      ...
      Object.keys(row).forEach(dStr => {
        ...
        this.items.push({ key, label: `Pair ${p},${p} vs ${d===11?'A':d}`, expected: row[d], type:'pair', playerCards: combo, dealerUp });
      });
    });
  ...
  private mastered(key:string){ return (this.mastery[key]||0) >= this.masteryTarget; }
  ...
  private makeCardFromValue(v:number){ return this.cardFromValue(v); }
  answer(decision: string){
    ...
    this.storage.recordSession({ ts: Date.now(), mode:'flash', correct: correct ? 1 : 0, attempts:1, expected:this.current.expected, chosen:decision, scenario:this.current.key, ms: dt });
  ...
  if(correct){ this.storage.incrementMastery(this.current.key); this.mastery[this.current.key] = (this.mastery[this.current.key]||0)+1; }
    setTimeout(()=>{
