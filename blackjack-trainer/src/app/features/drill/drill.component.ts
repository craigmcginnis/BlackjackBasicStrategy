...
export class DrillComponent {
  rules: RuleSet = { id: 'default', name: '6D S17', decks: 6, hitSoft17: false, doubleAfterSplit: true, lateSurrender: false };
  ...
  constructor(private engine: StrategyEngineService, private data: StrategyDataService){
    effect(()=>{ this.loadRules(); this.refreshExpected(); });
    this.next();
  }
  private loadRules(){
    ...
    this.actions = ['HIT','STAND','DOUBLE','SPLIT'].concat(this.rules.lateSurrender ? ['SURRENDER'] : [] ) as Decision[];
  }
  ...
  private genHand(){
    ...
    const history = this.storage.loadStats().history.slice(-300).filter(h=>h.mode==='drill' && h.scenario);
    const agg: Record<string,{a:number;c:number}> = {};
    for(const h of history){
      const key = h.scenario!;
      (agg[key] ||= {a:0,c:0}).a += h.attempts; (agg[key] ||= {a:0,c:0}).c += h.correct;
    }
    const weak = Object.entries(agg)
      .map(([k,v])=> ({k, acc: v.a? v.c / v.a : 0, a:v.a}))
      .filter(x=>x.a>=2 && x.acc < 0.75);
    let forced: { player:[{rank:string;value:number;suit:string},{rank:string;value:number;suit:string}], dealer:{rank:string;value:number;suit:string} } | null = null;
  if(weak.length && Math.random() < 0.35){
      ...
      if(type==='P'){
        ...
        const total = parseInt(totalOrRank,10); const second = total - 11; player = [this.card('A'), this.card(String(second))];
      } else { // hard
        const total = parseInt(totalOrRank,10);
        // simple decomposition
        if(total >= 12){ player = [this.card('10'), this.card(String(total-10))]; }
        else { player = [this.card(String(Math.max(2,total-2))), this.card(String(Math.min(10, total - Math.max(2,total-2))))]; }
      }
  ...
  answer(a: Decision){
    ...
    if(ok){ this.correct++; this.streak++; if(this.streak > this.bestStreak) this.bestStreak = this.streak; }
    else { this.streak = 0; }
    ...
    this.storage.recordSession({ ts: Date.now(), mode:'drill', correct: ok?1:0, attempts:1, expected:this.expected, chosen:a, scenario:this.scenarioKey(), ms: dt, usedHint: this.hintUsed });
  ...
  private buildExplanation(hand: EvaluatedHand, decision: Decision): string {
    ...
    if(decision === 'SURRENDER') return `Late surrender: Hard ${hand.total} vs dealer ${d} has poor expectation; surrender minimizes loss.`;
    if(decision === 'SPLIT' && hand.isPair && hand.pairRank){
      if(hand.pairRank === '8' || hand.pairRank === 'A') return `Always split ${hand.pairRank}${hand.pairRank} to improve outcomes.`;
      return `Splitting ${hand.pairRank}${hand.pairRank} vs dealer ${d} yields better EV per basic strategy.`;
    }
    if(decision === 'DOUBLE'){
      if(hand.isSoft) return `Soft ${hand.total} (A+${hand.total-11}) vs dealer ${d}: double while dealer is in vulnerable range.`;
  ...
  dealerLabel(v:number){ return v===11?'A':v; }
  cardFace(c:{rank:string; suit:string}){ return c.rank; }
  private scenarioKey(){
    if(!this.current) return '';
  ...
  showHint(){
    ...
    const classification = hand.isPair && hand.pairRank ? `${hand.pairRank}${hand.pairRank}` : (hand.isSoft ? `Soft ${hand.total}` : `Hard ${hand.total}`);
    ...
    if(hand.isPair && hand.pairRank){
      ...
      if(['A','8'].includes(pr)) guidance = 'Some pairs (like Aces or 8s) are often separated to avoid weak composite totals.';
      else if(pr === '5') guidance = 'Fives act like a strong hard 10; consider value of treating them as a total versus splitting.';
      else if(pr === '4') guidance = `Fours can be sensitive to whether double-after-split is ${this.rules.doubleAfterSplit ? 'allowed' : 'disallowed'} and the dealer card.`;
      else if(pr === '9') guidance = 'Nines are sometimes split against many dealer cards but not always against strong tens or Aces.';
      ...
      if(hand.total <= 17) guidance = 'Lower soft totals aim to leverage doubling windows against weak dealer cards (especially 4-6).';
      else if(hand.total === 18) guidance = 'Soft 18 is a border case: response shifts based on a narrow dealer range; aggression increases vs mid weak cards.';
      else guidance = 'High soft totals usually stand unless a special double opportunity exists under specific rules.';
      if(this.rules.hitSoft17) guidance += ' H17 rules slightly favor more aggressive doubles on some soft hands.';
    } else { // hard
      if(hand.total <= 8) guidance = 'Very low hard totals always continue drawing; risk is minimal.';
      else if(hand.total === 9) guidance = 'Hard 9 often looks for mid dealer weakness (3-6) to consider an aggressive option.';
      else if(hand.total === 10 || hand.total === 11) guidance = 'Hard 10/11 target broad dealer ranges for maximizing value.';
      else if(hand.total === 12) guidance = 'Hard 12 is a razor-edge total: standing only becomes attractive versus certain weak dealer cards.';
      else if(hand.total >= 13 && hand.total <= 16) guidance = dealerStrong ? 'Mid stiff total versus strong dealer often requires minimizing bust chances while considering surrender if available.' : 'Dealer weakness shifts mid stiff totals toward holding position.';
      ...
      if(this.rules.lateSurrender && (hand.total === 15 || hand.total === 16) && dealerStrong){
        guidance += ' With late surrender available, reducing expected loss can sometimes outweigh playing out a stiff hand here.';
      }
    }
