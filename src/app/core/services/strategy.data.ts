...
function buildStrategy(isH17: boolean, surrender: boolean, das: boolean): StrategyTables {
	...
	if (surrender) {
		[9, 10, 11].forEach((d) => (hard[16][d] = 'SURRENDER'));
	...
	['A2', 'A3'].forEach((k) => {
		for (let d = 5; d <= 6; d++) soft[k][d] = 'DOUBLE';
	});
	['A4', 'A5'].forEach((k) => {
		for (let d = 4; d <= 6; d++) soft[k][d] = 'DOUBLE';
	});
	...
	['A8', 'A9'].forEach((k) => {
		for (let d = 2; d <= 11; d++) soft[k][d] = 'STAND';
	});
	if (isH17) {
	...
	['2', '3'].forEach((p) => {
		for (let d = 2; d <= 7; d++) pairs[p][d] = 'SPLIT';
	});
